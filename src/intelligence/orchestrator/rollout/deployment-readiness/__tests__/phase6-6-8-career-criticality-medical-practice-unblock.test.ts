...
describe('Phase 6.6.8 career-criticality medical practice unblock readiness', () => {
  it('keeps MEDICAL_PRACTICE out of CriticalityDecisionType arrays', () => {
    ...
    expect(source).toContain("const irreversible: CriticalityDecisionType[] = ['CAREER_SWITCHING'];");
  });
...
function validEnv(): NodeJS.ProcessEnv {
  return {
    ...
    CAREEROS_STAGING_SHADOW_APPROVAL_ID: 'phase-6-6-8-test',
...
function approval(scope: 'CANARY_LIVE' | 'FULL_LIVE') {
  return createRouteShadowApproval({
    ...
    maxExecutions: 1,
    reason: 'Phase 6.6.8 live-mode block regression.',
