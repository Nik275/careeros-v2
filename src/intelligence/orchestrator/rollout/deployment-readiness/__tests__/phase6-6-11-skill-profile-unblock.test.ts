...
describe('Phase 6.6.11 SkillProfile unblock readiness', () => {
  it('documents the stale SkillProfile import as removed from CareerTransitionGraphV1', () => {
    ...
    expect(source).not.toContain('SkillProfile');
  });

  it('documents CareerSkillProfile as the canonical richer skill-profile type', () => {
    const source = readFileSync(
      join(process.cwd(), 'src/intelligence/skill-taxonomy/SkillTaxonomyV1.ts'),
      'utf8'
    );

    expect(source).toContain('export interface CareerSkillProfile');
    expect(source).toContain('requiredSkills: CareerSkillRequirement[]');
    expect(source).toContain('primaryCategories: SkillCategory[]');
  });
...
function validEnv(): NodeJS.ProcessEnv {
  return {
    ...
    CAREEROS_STAGING_SHADOW_APPROVAL_ID: 'phase-6-6-11-test',
...
function approval(scope: 'CANARY_LIVE' | 'FULL_LIVE') {
  return createRouteShadowApproval({
    ...
    maxExecutions: 1,
    reason: 'Phase 6.6.11 live-mode block regression.',
