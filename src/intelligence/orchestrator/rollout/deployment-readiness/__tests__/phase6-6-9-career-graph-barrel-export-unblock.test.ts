...
describe('Phase 6.6.9 career-graph barrel export unblock readiness', () => {
  it('keeps career-graph type-only symbols in export type blocks', () => {
    ...
    expect(runtimeExportBlocks).not.toMatch(/^\s*GraphBuilderOptions,\s*$/m);
  });

  it('keeps career-graph runtime exports in runtime export blocks', () => {
    const source = readFileSync(
      join(process.cwd(), 'src/intelligence/career-graph/index.ts'),
      'utf8'
    );

    expect(source).toContain('CareerGraphEngine');
    expect(source).toContain('GraphBuilder');
    expect(source).toContain('PREDEFINED_PATHWAYS');
    expect(source).toContain('DEFAULT_CAREER_GRAPH_CONFIG');
  });
...
function validEnv(): NodeJS.ProcessEnv {
  return {
    ...
    CAREEROS_STAGING_SHADOW_APPROVAL_ID: 'phase-6-6-9-test',
...
function approval(scope: 'CANARY_LIVE' | 'FULL_LIVE') {
  return createRouteShadowApproval({
    ...
    maxExecutions: 1,
    reason: 'Phase 6.6.9 live-mode block regression.',
