...
describe('Phase 6.6.3 belief confidence build unblock readiness', () => {
  it('keeps ConfidenceLevel as a type-only Bayesian belief alias', () => {
    const source = readFileSync(
      join(process.cwd(), 'src/intelligence/bayesian-belief-engine/types.ts'),
      'utf8'
    );

    expect(source).toContain("export type BeliefConfidenceLevel = 'low' | 'moderate' | 'high' | 'veryHigh';");
    expect(source).toContain('export type ConfidenceLevel = BeliefConfidenceLevel;');
    expect(source).not.toMatch(/export\s+(enum|const)\s+ConfidenceLevel/);
  });

  it('keeps the Bayesian barrel exporting ConfidenceLevel as a type', () => {
    const source = readFileSync(
      join(process.cwd(), 'src/intelligence/bayesian-belief-engine/index.ts'),
      'utf8'
    );

    expect(source).toContain('BeliefConfidenceLevel');
...
function validEnv(): NodeJS.ProcessEnv {
  return {
    ...
    CAREEROS_STAGING_SHADOW_APPROVAL_ID: 'phase-6-6-3-test',
...
function approval(scope: 'CANARY_LIVE' | 'FULL_LIVE') {
  return createRouteShadowApproval({
    ...
    maxExecutions: 1,
    reason: 'Phase 6.6.3 live-mode block regression.',
