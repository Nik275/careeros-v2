...
describe('Phase 6.6.7 calibration barrel export unblock readiness', () => {
  it('keeps calibration barrel type-only symbols in export type blocks', () => {
    ...
    expect(runtimeExportBlocks).not.toContain('UnifiedCalibrationProfile');
  });

  it('keeps calibration barrel runtime exports in runtime export blocks', () => {
    const source = readFileSync(
      join(process.cwd(), 'src/intelligence/calibration/index.ts'),
      'utf8'
    );
    ...
    expect(source).toContain('ReliabilityBand');
    expect(source).toContain('CalibrationEngine');
    expect(source).toContain('RecommendationCalibrationEngine');
    expect(source).toContain('DEFAULT_RECOMMENDATION_CONFIG');
  });
...
function validEnv(): NodeJS.ProcessEnv {
  return {
    ...
    CAREEROS_STAGING_SHADOW_APPROVAL_ID: 'phase-6-6-7-test',
...
function approval(scope: 'CANARY_LIVE' | 'FULL_LIVE') {
  return createRouteShadowApproval({
    ...
    maxExecutions: 1,
    reason: 'Phase 6.6.7 live-mode block regression.',
