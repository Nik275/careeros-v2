...
import { CalibrationStatus, ReliabilityBand } from '../../../../calibration/calibration-types';
...
describe('Phase 6.6.6 calibration status build unblock readiness', () => {
  it('keeps GOOD out of CalibrationStatus and uses the canonical positive status', () => {
  ...
  it('keeps calibration-tests.ts from referencing nonexistent CalibrationStatus.GOOD', () => {
    ...
    expect(source).toContain('status: CalibrationStatus.WELL_CALIBRATED');
    expect(source).toContain('reliabilityBand: ReliabilityBand.GOOD');
  });
...
function validEnv(): NodeJS.ProcessEnv {
  return {
    ...
    CAREEROS_STAGING_SHADOW_APPROVAL_ID: 'phase-6-6-6-test',
...
function approval(scope: 'CANARY_LIVE' | 'FULL_LIVE') {
  return createRouteShadowApproval({
    ...
    maxExecutions: 1,
    reason: 'Phase 6.6.6 live-mode block regression.',
