/**
 * @fileoverview Safe defaults for Phase 5.7 staging CANARY_SHADOW runs.
 */

import type { StagingShadowRunConfig } from './StagingShadowRunTypes';

export const DEFAULT_STAGING_SHADOW_RUN_CONFIG: StagingShadowRunConfig = Object.freeze({
  runId: 'staging-shadow-disabled',
  enabled: false,
  environment: 'unknown',
...
export function createStagingShadowRunConfig(
  overrides: Partial<StagingShadowRunConfig> = {}
): StagingShadowRunConfig {
  const config = {
    ...DEFAULT_STAGING_SHADOW_RUN_CONFIG,
    ...overrides,
    allowedEnvironments:
      overrides.allowedEnvironments ?? DEFAULT_STAGING_SHADOW_RUN_CONFIG.allowedEnvironments,
    allowedFlows: overrides.allowedFlows ?? DEFAULT_STAGING_SHADOW_RUN_CONFIG.allowedFlows,
  };
