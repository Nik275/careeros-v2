...
function createDecisionInput(options: DecisionOption[] = [createOption()]): DecisionInput {
  ...
  return {
    id: 'decision-psychology-profile-contract' as DecisionInput['id'],
    type: 'CAREER_CHOICE',
    studentId: 'student-psychology-profile-contract-test',
    ...
    careerRecommendations: {
      studentProfileId: 'student-psychology-profile-contract-test',
      topRecommendations: [],
...
describe('Decision PsychologyProfile type-surface contract', () => {
  ...
  it('PsychologyProfile resolves from the canonical student-domain type surface', () => {
    ...
    expect(decisionProfile.curiosity).toBe(0.81);
  });

  it('PsychologyProfile remains type-only in the decision-intelligence runtime surface', async () => {
    const decisionIntelligenceModule = await import('../index');

    expect(Object.prototype.hasOwnProperty.call(decisionIntelligenceModule, 'PsychologyProfile')).toBe(false);
  });

  it('keeps DecisionInput profile fields type-safe for valid analysis', () => {
    const engine = createDecisionIntelligenceEngine();
    const input = createDecisionInput([createOption()]);

    expect(input.psychologyProfile.riskTolerance).toBe(0.52);
