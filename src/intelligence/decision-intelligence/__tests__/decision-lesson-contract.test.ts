...
function createLongitudinalPattern(): LongitudinalPattern {
  const pattern: SuccessPattern = {
    id: 'pattern-lesson-contract' as SuccessPattern['id'],
    pattern: 'Students who convert mentor lessons into experiments improve decision quality.',
    category: 'LEARNING',
    frequency: {
      count: 4,
      total: 10,
    ...
    behaviors: ['turns advice into tests', 'tracks evidence', 'compares outcomes'],
    enablingConditions: ['access to mentors', 'time to explore'],
    blockingConditions: ['hard deadline', 'no exploration budget'],
    sourceJourneys: ['journey-lesson-contract' as SuccessPattern['sourceJourneys'][number]],
    counterExamples: [],
    relatedPatterns: [],
  };
...
function createLesson(): ExtractedLesson {
  return {
    id: 'lesson-decision-contract' as ExtractedLesson['id'],
    ...
    evidence: [
      {
        journeyId: 'journey-lesson-contract' as ExtractedLesson['evidence'][number]['journeyId'],
    ...
    relatedLessons: [],
    sourceJourneys: ['journey-lesson-contract' as ExtractedLesson['sourceJourneys'][number]],
...
function createDecisionInput(options: DecisionOption[] = [createOption()]): DecisionInput {
  ...
  return {
    id: 'decision-lesson-contract' as DecisionInput['id'],
    type: 'CAREER_CHOICE',
    studentId: 'student-lesson-contract-test',
    description: 'Choose a career path while applying mentor lessons.',
    options,
    context: {
    ...
    careerRecommendations: {
      studentProfileId: 'student-lesson-contract-test',
      topRecommendations: [],
...
describe('Decision Lesson type-surface contract', () => {
  ...
  it('Lesson resolves from the canonical mentor-intelligence type surface', () => {
    const mentorLesson: ExtractedLesson = createLesson();
    const decisionLesson: Lesson = mentorLesson;

    expect(decisionLesson.lesson).toContain('Validate assumptions');
    expect(decisionLesson.category).toBe('DECISION_MAKING');
    expect(decisionLesson.type).toBe('VALIDATION_INSIGHT');
  });

  it('Lesson remains type-only in runtime module surfaces', async () => {
    ...
    expect(Object.prototype.hasOwnProperty.call(decisionIntelligenceModule, 'Lesson')).toBe(false);
    expect(Object.prototype.hasOwnProperty.call(mentorTypesModule, 'ExtractedLesson')).toBe(false);
  });

  it('keeps DecisionInput lesson fields type-safe for valid analysis', () => {
    const engine = createDecisionIntelligenceEngine();
    const input = createDecisionInput([createOption()]);
    const lesson = input.lessons?.[0];

    expect(lesson?.lesson).toContain('reversible experiment');
    expect(lesson?.sourceJourneys.length).toBe(1);
  ...
  it('preserves Phase 6.6.Z LongitudinalPattern contract', () => {
    ...
    expect(memory[0]?.pattern).toContain('mentor lessons');
