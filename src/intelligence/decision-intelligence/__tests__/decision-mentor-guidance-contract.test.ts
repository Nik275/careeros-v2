...
function createDecisionInput(options: DecisionOption[] = [createOption()]): DecisionInput {
  return {
    id: 'decision-mentor-guidance-contract' as DecisionInput['id'],
    type: 'CAREER_CHOICE',
    studentId: 'student-mentor-guidance-contract-test',
    ...
    careerRecommendations: {
      studentProfileId: 'student-mentor-guidance-contract-test',
      topRecommendations: [],
...
describe('Decision mentor guidance contract', () => {
  it('imports successfully', () => {
    expect(createDecisionIntelligenceEngine).toBeTypeOf('function');
  });

  it('analyze() returns mentorGuidance for one valid option', () => {
  ...
  it('analyze() returns mentorGuidance for multiple valid options', () => {
    const engine = createDecisionIntelligenceEngine();
    ...
    expect(analysis.mentorGuidance).toContain(analysis.primaryRecommendation.optionLabel);
  });

  it('mentorGuidance has the required string shape', () => {
    const engine = createDecisionIntelligenceEngine();
    const analysis = engine.analyze(createDecisionInput([createOption()]));

    expect(analysis.mentorGuidance).toBeTypeOf('string');
    expect(analysis.mentorGuidance).toContain('Discuss');
    expect(analysis.mentorGuidance).toContain('Monitor regret risk');
  });

  it('mentorGuidance is not null or undefined', () => {
    const engine = createDecisionIntelligenceEngine();
    const analysis = engine.analyze(createDecisionInput([createOption()]));

    expect(analysis.mentorGuidance).not.toBeNull();
    expect(analysis.mentorGuidance).not.toBeUndefined();
  });
