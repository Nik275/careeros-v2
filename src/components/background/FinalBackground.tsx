...
const FloatingSparkles = memo(function FloatingSparkles() {
  // Generate 55 particles, keeping center clean
  const particles = [];
  for (let i = 0; i < 55; i++) {
    let left, top;
    let attempts = 0;
    ...
    <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 3 }}>
      {particles.map((p) => (
        <div
          ...
          style={{
            ...
            animation: `floatSparkle ${p.duration}s ease-in-out ${p.delay}s infinite alternate`,
            zIndex: 3,
          }}
        />
      ))}
      <style jsx>{`
        @keyframes floatSparkle {
          ...
          100% { transform: translate(${particles[0]?.moveX || 4}px, -${particles[0]?.moveY || 3}px); }
        }
      `}</style>
    </div>
  );
...
export const FinalBackground = memo(function FinalBackground() {
  return (
    <div
